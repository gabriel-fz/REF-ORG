...
function editarReferencias(idCit) {
  let txtar = document.querySelector(`textarea#txtReferenciac${idCit}`).value
  let txtac = document.querySelector(`textarea#txtCitacao${idCit}`).value
  txtar && txtac ? db.get('citacoes').find({ id: `${idCit}` }).assign({ textCitacao: `${txtac}`, textReferencia: `${txtar}` }).write()
    : alert("Campos não preenchidos")
}
...
function listaCitacoes() {
  ...
  for (let contador in cits) {
    mostraCitacoes += `<div class="card my-2">
                            <div class="row no-gutters">
                              <div class="col-2 texto-centro">
                                <button class="btn btn-two btn-lg mx-2" data-toggle="modal" data-target="#Modal${cits[contador].id}"><i class="fas fa-pen fa-sm"></i></button>
                          ...
                          <div class="modal fade" id="Modal${cits[contador].id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg">
                              <div class="modal-content">
                                <div class="modal-header">
                                  <h5 class="modal-title">Editar</h5>
                                ...
                                <div class="modal-body">
                                  <form class="text-center">
                                    <div class="form-group">
                                      <textarea class="form-control mt-2" id="txtReferenciac${cits[contador].id}" rows="3">${cits[contador].textCitacao}</textarea>
                                    </div>
                                    <div class="form-group">
                                      <textarea class="form-control mt-2" id="txtCitacao${cits[contador].id}" rows="2">${cits[contador].textReferencia}</textarea>
                                    </div>
                                  </form>
                                </div>
                                <div class="modal-footer">
                                  <button type="button" class="btn btn-one" onclick="editarReferencias(${cits[contador].id})">Salvar</button>
                                  <button type="button" class="btn btn-one" data-dismiss="modal">Cancelar</button>
                                </div>
                              </div>
                            </div>
